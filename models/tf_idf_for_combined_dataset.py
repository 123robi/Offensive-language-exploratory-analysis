...
class TFIDFCombined(AbstractProcessor):
    dataset: Series
    ...
    def preprocess(self):
        ...
        text = keep_chars_only(text)
        text = remove_additional_spaces(text)
        text.dropna(inplace=True)
        # get tokens, remove stopwords and stem

        text_tokens = stemming(remove_stopwords(get_tokens(text), self.additional_stopwords))
...
class LogisticRegressionModel(ModelCombined):
    def __init__(self, tfidf: TFIDFCombined):
...
class MultinomialNaiveBayesModel(ModelCombined):
    def __init__(self, tfidf: TFIDFCombined):
        super().__init__(tfidf)
...
class GaussianNaiveBayesModel(ModelCombined):
    def __init__(self, tfidf: TFIDFCombined):
        super().__init__(tfidf)
...
class RandomForestClassifierModel(ModelCombined):
    def __init__(self, tfidf: TFIDFCombined):
        super().__init__(tfidf)
...
class BernoulliNaiveBayesModel(ModelCombined):
    def __init__(self, tfidf: TFIDFCombined):
        super().__init__(tfidf)
